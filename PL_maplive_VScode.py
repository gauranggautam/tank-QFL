...
class PLMapGUI:
    ...
    def go_to_position(self):
        try:
            ...
            amc.move.setControlTargetPosition(0, int(x * 1000));wait_until_stable(amc_dev=amc,axis=0)
            amc.move.setControlTargetPosition(2, int(y * 1000));wait_until_stable(amc_dev=amc,axis=2)
    ...
    def go_to_up(self): 
        try:
            y_act = amc.move.getPosition(2) / 1000
            amc.move.setControlTargetPosition(2, int((y_act+ystpp) * 1000));wait_until_stable(amc_dev=amc,axis=2)
            y_act = amc.move.getPosition(2) / 1000
            self.y_act_var.set(f"{y_act:.3f}")
        except Exception as e:
                    messagebox.showerror("Error", str(e))
    ...
    def go_to_left(self): 
        try:
            x_act = amc.move.getPosition(0) / 1000
            amc.move.setControlTargetPosition(0, int((x_act-xstpp) * 1000));wait_until_stable(amc_dev=amc,axis=0)   
            x_act = amc.move.getPosition(0) / 1000
            self.x_act_var.set(f"{x_act:.3f}")
    ...
    def go_to_fup(self): 
        try:
            ...
            amc.move.setControlTargetPosition(1, int((f_act+fstpp) * 1000));wait_until_stable(amc_dev=amc,axis=1)   
            f_act = amc.move.getPosition(1) / 1000
