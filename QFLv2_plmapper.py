...
def run_focus_sweep(fbase=None, fstep=0.1, fsize=50, movetobest=True,showplt=True, engaged=False, sn=None, d1=None, d2=None, amc=None):
    try:
        if amc is None:
            amc = start_attocube()
        if sn is None:
        ...
        if movetobest:
            ...
            wait_until_stable(amc, 1)
            print(f"\nMoved to Best focus at: {bestf:.2f} µm")
        plt.ioff()
        if showplt:
            plt.show()
...
def run_pl_scan(center_x=0,center_y=0,center_f=None,focus_sweep=False,x_size=5,y_size=5,step=1,detector_config=2,multi_pl=False,logz=False,out_dir_base='C:/Users/iq-qfl/Documents/Gaurang/GitHub/git_codes/PlotBasic/Output/PLmaps',show_plot=True,sn=None, amc=None, d1=None, d2=None):
    """
    ...
    try:
        ...
        if focus_sweep:
            print(f"Sweeping focus (Z-axis) around: {fnow} µm")
            best_f=run_focus_sweep(fbase=fnow, sn=sn, amc=amc, d1=d1, d2=d2,movetobest=True, showplt=False,fsize=40, engaged=True)
            fnow=best_f    
        ...
        data_file_handle.write(f'#readback numpy shape for line part: {x_pos.shape[0]}, {y_pos.shape[0]}\n')
        ...
        cnt = sn.getCountRates()
        total = cnt[d1] + cnt[d2]
        max_int, best_x, best_y = total, x_start, y_start
        ...
        for i, y in enumerate(y_pos):
            ...
            for j_scan, x in enumerate(x_scan_pos):
                ...
                try:
                    ...
                    total = cnt[d1] + cnt[d2]
                    total = np.log10(total + 1) if logz else total
...
def run_pl_position_optimizer(focussweep=False, scan_size=2,scan_step=0.1,amc=None, sn=None,engaged=False,movetoxy=True):
    try:
        if sn is None:
            sn, d1, d2 = start_apds(detector_config=2)
        if amc is None:
        ...
        bx, by, bf = run_pl_scan(center_x=x_now, center_y=y_now, center_f=f_now, x_size=scan_size, y_size=scan_size,step=scan_step,focus_sweep=True if focussweep else False,sn=sn, d1=d1, d2=d2,amc=amc)
        if movetoxy:
            amc = start_attocube()          
...
def plot_plmap(input, mode='custom', flog=False, xi=0, yi=2, zi=6, id='Plot',invxy=False):
    ...
    if mode in ['vscode']:
        X, Y, Z= make_grid(v[x_index], v[y_index],v[z_index])
    else:
        X, Y = v[x_index], v[y_index]
        Z = np.log10(v[z_index]) if flog else v[z_index]
    
    plmap = ax.pcolor(X,Y,Z, cmap='plasma')
