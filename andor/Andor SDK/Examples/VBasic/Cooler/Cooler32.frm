...
Begin VB.Form Form1 
   BackColor       =   &H00808000&
   Caption         =   "Instaspec Visual Basic Example Drivers:- Detector Cooler System"
   ClientHeight    =   6030
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8295
   ...
   Begin VB.PictureBox Picture1 
      ...
      Width           =   3990
   End
   Begin VB.CommandButton pbCoolerOff 
      Caption         =   "C&ooler Off"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
      ...
      Width           =   990
   End
   Begin VB.CommandButton pbCoolerOn 
      Caption         =   "&Cooler On"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
      ...
      Width           =   990
   End
   Begin VB.TextBox ebTemperature 
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
'This application demonstrates how to acquire a Single Scan,
'Full Vertically Binned (FVB) scan using InstaSpec.
'
...
'**************************************************************
  ...
  PrintStatusMsg ("* FULL VERTICAL BINNING * SINGLE SCAN *")
...
'**************************************************************
  ...
  If (vbInitialize() = LOCALERROR) Then
    Exit Sub
  End If
  If (vbGetTempRange() = LOCALERROR) Then
    Exit Sub
  End If
...
Sub EnableAcqButtons(bEnable As Long)
'*************************************************************************
' Enable/Disable the close button.  Used whenever the temperature is
' varying.
'*************************************************************************
  pbClose(2).Enabled = bEnable
End Sub
...
Function vbCoolerOFF()
'*************************************************************************
' Switches OFF the InstaSpec CCD sensor cooler.
'*************************************************************************
  vbCoolerOFF = Not LOCALERROR
  test = CoolerOFF()
  If (test <> DRV_SUCCESS) Then
    vbCoolerOFF = LOCALERROR
    buffer = "Temperature ERROR: "
    Select Case (test)
      Case DRV_NOT_INITIALIZED
        buffer = buffer & "System not initialized."
      Case DRV_VXDNOTLOAD
        buffer = buffer & "VxD (atmcd95.vxd) not loaded."
      Case DRV_COFNOTLOAD
        buffer = buffer & " 'pci_29k.cof' not loaded."
      Case Else
...
Function vbCoolerON()
'*************************************************************************
' Switches ON the InstaSpec CCD sensor cooler.
'*************************************************************************
  vbCoolerON = Not LOCALERROR
  test = CoolerON()
  If (test <> DRV_SUCCESS) Then
    vbCoolerON = LOCALERROR
    buffer = "Temperature ERROR: "
    Select Case (test)
      Case DRV_NOT_INITIALIZED
        buffer = buffer & "System not initialized."
...
'***************************************************************
  ...
  If (status = DRV_IDLE) Then
    If (LoadData() = Not LOCALERROR) Then
      StartTimer = XOFF
    End If
  Else
    vbGetStatus = LOCALERROR
...
Function vbGetTemperature(temperature As Long)
'*************************************************************************
' Interrogates InstaSpec for the current sensor temperature.
'*************************************************************************
  vbGetTemperature = Not LOCALERROR
  test = GetTemperature(temperature)
  If (test <> DRV_SUCCESS) Then
    buffer = "Temperature ERROR: "
    vbGetTemperature = LOCALERROR
    Select Case (test)
      Case DRV_TEMPERATURE_OFF
        buffer = buffer & "Cooler is OFF."
        CoolerOffFlag = XOFF
        CoolerOnFlag = XOFF
      Case DRV_ERROR_ACK
      ...
      Case DRV_TEMPERATURE_STABILIZED
        buffer = buffer & "Temperature has stabilized"
        buffer = buffer & "Its OK to acquire data now."
        vbGetTemperature = DRV_TEMPERATURE_STABILIZED
  ...
  Else
    buf = ""
    buffer = "Current temperature is " & Str(temperature) & " 'C"
    PrintStatusMsg (buffer)
  End If
End Function

Function vbGetTempRange()
'***********************************************************************
' Gets the possible temperature range for the CCD sensor.
'***********************************************************************
  Dim MinTemp As Long
  Dim MaxTemp As Long
  
  vbGetTempRange = Not LOCALERROR
  test = GetTemperatureRange(MinTemp, MaxTemp)
  If (test <> DRV_SUCCESS) Then
    buffer = "Temperature ERROR: "
    vbGetTempRange = LOCALERROR
    Select Case (test)
      Case DRV_INIERROR
        buffer = buffer & "Unable to load 'Detector.ini'."
      Case DRV_NOTINITIALIZED
        buffer = buffer & "system not initialized."
      Case Else
        buffer = buffer & "Unknown Initialize error."
    End Select
  Else
    buffer = newLine & "Temperature range for this sensor is: "
    buffer = buffer & Str(MinTemp) & " to "
    buffer = buffer & Str(MaxTemp) & " 'C" & newLine
  End If
  PrintStatusMsg (buffer)
End Function
...
'****************************************************************
  ...
  Else
    buf = "Set Exposure Time to " & Str(exposure) & " secs"
  End If
  ebStatusBox.text = buf
End Function
...
'**************************************************************
' Configure InstaSpec to Full Vertical Binning (FVB).
'**************************************************************
  vbSetReadout = Not LOCALERROR
  test = SetReadMode(0)              'Full Vertical Binning
  If test = DRV_SUCCESS Then
    buffer = "Set Readout Mode to Full Vertical Binning"
  Else
...
Function vbSetTemperature()
'*************************************************************************
' Sets the CCD sensor operating temperature.  (Note: still need to switch
' on the cooler.)
'*************************************************************************
  vbSetTemperature = Not LOCALERROR
  temp = Val(ebTemperature.text)
  test = SetTemperature(temp)
  If (test <> DRV_SUCCESS) Then
    vbSetTemperature = LOCALERROR
    buffer = "Temperature ERROR: "
    Select Case (test)
      Case DRV_NOT_INITIALIZED
        buffer = buffer & "System not initialized."
      Case P1INVALID
        buffer = buffer & "Invalid temperature."
      Case Else
        buffer = buffer & "Unknown error."
    End Select
    PrintStatusMsg (buffer)
...
Function LoadData()
'**************************************************************
' After vbGetStatus() confirms the data acquisition is
...
'**************************************************************
  ...
  ReDim aData(nXPixels)
  test = GetAcquiredData(aData(1), nXPixels)
...
Sub MonitorTemperatureFall()
'*********************************************************************
' Monitors the temperature fall against the desired operating
' temperature
'*********************************************************************
  DesiredTemperature = Val(ebTemperature.text)
  Dim CurrentTemp As Long
  test = vbGetTemperature(CurrentTemp)
  If (test = LOCALERROR) Then
    Exit Sub
  End If
  buffer = "Current Temperature is: " & Str(CurrentTemp) & " 'C"
  'If (CurrentTemp <= DesiredTemperature) Then
  If (test = DRV_TEMPERATURE_STABILIZED) Then
    buffer = buffer & newLine & "Reached Set Temperature."
    CoolerOnFlag = XOFF
    CoolerOffFlag = XOFF
    EnableAcqButtons (True)
    PrintStatusMsg buffer
    Period = 1
    Exit Sub
  End If
  Period = Period + 1
  result = Period Mod 100
  If (result = 0) Then
    PrintStatusMsg (buffer)
  End If
End Sub

Sub MonitorTemperatureRise()
'*************************************************************************
' When the InstaSpec system is shut down the sensor operating temperature
' MUST EXCEED 0'C to prevent heating up too quickly.  This function
' monitors the temperature rise whenever the cooler is switched off and
' will only allow the application to close (via the "Close" button) when
' the temperature has risen above 0'C.
'*************************************************************************
  RiseTemperature = 0
  Dim CurrentTemp As Long
  If (vbGetTemperature(CurrentTemp) = LOCALERROR) Then
    Exit Sub
  End If
  If (CurrentTemp >= RiseTemperature) Then
    buffer = "Temperature has risen sufficiently."
    buffer = buffer & newLine
    buffer = buffer & "Its now OK to close down the application."
    CoolerOffFlag = XOFF
    EnableAcqButtons (True)
    PrintStatusMsg (buffer)
  End If
...
'**************************************************************
' Closes down the FVB application
'**************************************************************
  test = ShutDown()
  If (test <> DRV_SUCCESS) Then
...
Sub pbCoolerOff_Click()
'************************************************************
' Response function to the "Cooler Off" button.
' Care is needed to remove any superseded timer
' before creating a new one.
'
' Tell Timer() which procedure to monitor by setting a flag.
'************************************************************
  If (vbCoolerOFF() <> LOCALERROR) Then
    CoolerOnFlag = XOFF
    CoolerOffFlag = XON
  End If
End Sub

Sub pbCoolerOn_Click()
'************************************************************
' Response function to the "Cooler On" button.
' Care is needed to remove any superseded timer
' before creating a new one.
'
' Tell Timer() which procedure to monitor by setting a flag.
'************************************************************
  If (vbSetTemperature() = LOCALERROR) Then
    Exit Sub
  End If
  If (vbCoolerON() = LOCALERROR) Then
    Exit Sub
  Else
    CoolerOnFlag = XON
    CoolerOffFlag = XOFF
    EnableAcqButtons (False)
    buf = ""
    buffer = "Sensor temperature is being lowered" & newLine
    buffer = buffer & "and until it has reached the desired" & newLine
    buffer = buffer & "temperature you will NOT be able to" & newLine
    buffer = buffer & "acquire data."
    Dim CurrentTemp As Long
    test = vbGetTemperature(CurrentTemp)
    If (test = LOCALERROR) Then
        Exit Sub
...
'*************************************************************************************
'This routine is the heart of the fvb application.  It configures the InstaSpec
...
'**************************************************************
  ...
  MaxY = aData(1)
  For i = 1 To nXPixels
    If (aData(i) >= MaxY) Then
      MaxY = aData(i)
    End If
    If (aData(i) <= MinY) Then
      MinY = aData(i)
    End If
    DataDisplay.GraphData = aData(i)
  Next i
...
'**************************************************************
  If (StartTimer = XON) Then
    If (vbGetStatus() = True) Then
    End If
  End If

  If (CoolerOnFlag = XON) Then
    MonitorTemperatureFall
  End If
    
  If (CoolerOffFlag = XON) Then
    MonitorTemperatureRise
  End If
End Sub
