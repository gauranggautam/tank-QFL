...
Begin VB.Form Form1 
   BackColor       =   &H00808000&
   Caption         =   "AndorSDK Visual Basic Example Drivers:- EMCCD FVB"
   ClientHeight    =   6735
   ClientLeft      =   135
   ClientTop       =   420
   ClientWidth     =   8295
   ...
   Begin VB.TextBox ebExposureTime 
      ...
      Width           =   1692
   End
   Begin VB.Label stGain 
      BackStyle       =   0  'Transparent
      Caption         =   "Gain Level (0-255)"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
'Full Vertically Binned (FVB) using AndorSDK with an EMCCD.
'
...
'source).
'
'The Gain of the EMCCD can be set to a value from 0 to 255.
'
...
'**************************************************************
' Configure AndorSDK to Full Vertical Binning (FVB).
'**************************************************************
...
Function vbSetGain()
'******************************************************************
' Sets the gain of the EMCCD system through a call to SetEMCCDGain
'******************************************************************
  vbSetGain = Not LOCALERROR
  gain = Val(ebGain.text)
  test = SetEMCCDGain(gain)
  If (test <> DRV_SUCCESS) Then
    vbSetGain = LOCALERROR
    buffer = "Set Gain ERROR:"
    Select Case (test)
      Case DRV_NOT_INITIALIZED
        buffer = buffer & " AndorSDK system isn't initialized."
      Case DRV_ACQUIRING
        buffer = buffer & " Set Gain Warning" & newLine
        buffer = buffer & "AndorSDK is currently acquiring data."
      Case DRV_ERROR_ACK
        buffer = buffer & newLine & "Unable to communicate with card."
      Case DRV_P1INVALID
        buffer = buffer & "Gain value invalid."
      Case DRV_I2CTIMEOUT
        buffer = buffer & "I2C command timed out."
      Case DRV_I2CDEVNOTFOUND
        buffer = buffer & "I2C device not found."
      Case Else
        buffer = buffer & "Unknown Set Gain Error."
    End Select
    PrintStatusMsg (buffer)
  End If
End Function
...
'*************************************************************************************
'This routine is the heart of the fvb application.  It configures the AndorSDK
