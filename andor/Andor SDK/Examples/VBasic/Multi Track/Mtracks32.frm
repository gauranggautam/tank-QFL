...
Begin VB.Form Form1 
   BackColor       =   &H00808000&
   Caption         =   "Instaspec Visual Basic Example Drivers:- MultiTracks"
   ClientHeight    =   7245
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8295
   ...
   Begin VB.TextBox ebShutterTime 
      ...
      Width           =   972
   End
   Begin VB.TextBox ebTrackOffset 
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.Frame Frame1 
      ...
      End
   End
   Begin VB.TextBox ebTrackHeight 
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.Label lblTrackHeight 
      ...
      Width           =   2415
   End
   Begin VB.Label lblTrackOffset 
      BackColor       =   &H00808000&
      Caption         =   "Track &offset"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
'acquire a Single Scan, MultiTracks image.
...
'**************************************************************
  ...
  PrintStatusMsg ("* MULTITRACKS * SINGLE SCAN * INTERNAL TRIGGER *")
...
' 4) Set binning pattern to MultiTracks.
'**************************************************************
...
'***********************************************************
  ...
  numTracks = Val(ebNumTracks.text)
...
Function vbSetMultiTracks()
'**************************************************************
' The binning pattern of the InstaSpec system is configured to be
' a set of tracks (ie MultiTrack mode).  This routine defines their
' heights and spacing.
' The SetMultiTrack function will also return the bottom row of
' the bottom track and the gap between tracks ( in CCD rows).
'**************************************************************
    
  Dim bottom As Long
  Dim gap As Long
  numTracks = Val(ebNumTracks.text)
  TrackHeight = Val(ebTrackHeight.text)
  TrackOffset = Val(ebTrackOffset.text)
  vbSetMultiTracks = Not Local_ERROR
    
  test = SetMultiTrack(numTracks, TrackHeight, TrackOffset, bottom, gap)
  If (test <> DRV_SUCCESS) Then
    vbSetMultiTracks = Local_ERROR
    buffer = "MultiTrack ERROR: "
    Select Case (test)
      Case DRV_P1INVALID
        buffer = buffer & "Number of tracks invalid."
      Case DRV_P2INVALID
        buffer = buffer & "Track height invalid."
      Case DRV_P3INVALID
        buffer = buffer & "Offset invalid."
      Case Else
...
'**************************************************************
' Configure InstaSpec binning pattern to MultiTracks.
'**************************************************************
  vbSetReadout = Not LOCALERROR
  test = SetReadMode(1)              'MultiTracks
  If test = DRV_SUCCESS Then
    buffer = "Set Readout Mode to MultiTracks"
  Else
...
'**************************************************************
' Closes down the MultiTracks application
'**************************************************************
  test = ShutDown()
  If (test <> DRV_SUCCESS) Then
...
'*************************************************************************************
'This routine is the heart of the MultiTracks application.  It configures the InstaSpec
...
'**************************************************************************************
  ...
  If (StartTimer = XOFF) Then
    ...
    If (vbSetShutter() = LOCALERROR) Then
      Exit Sub
    End If
    If (vbSetMultiTracks() = LOCALERROR) Then
      Exit Sub
    End If
