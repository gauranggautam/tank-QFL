...
Begin VB.Form Form1 
   BackColor       =   &H00808000&
   Caption         =   "Andor iVac Visual Basic Example Drivers:- Full Vertical Binning"
   ClientHeight    =   5940
   ClientLeft      =   135
   ClientTop       =   420
   ClientWidth     =   8295
...
'This application demonstrates how to acquire in Real Time,
'Full Vertically Binned (FVB) using Andor iVac.
'
'The user may specify the exposure time and type of Trigger
'pulse (either Internal or External).  Internal Trigger is
'the simplest option and allows the computer to generate its
'own trigger pulse.  In External trigger mode the user MUST
'supply their own triggering pulse (eg from a laser or lamp
'source).
'
'All driver calls are in functions preceded by the
...
'**************************************************************
  ...
  PrintStatusMsg ("* FULL VERTICAL BINNING * REAL TIME *")
...
'**************************************************************
  ...
  If (vbSetReadout() = LOCALERROR) Then
    Exit Sub
  End If
  If (vbSetCooling() = LOCALERROR) Then
    Exit Sub
  End If
End Sub
...
'**************************************************************
  ...
  test = SetAcquisitionMode(5)      'Run Till Abort
  If test = DRV_SUCCESS Then
    buffer = "Set Acquisition Mode to Real Time"
  Else
...
Function vbSetCooling()
'**************************************************************
' Configure Andor iVac Cooling.
'**************************************************************
  vbSetCooling = Not LOCALERROR
  test = SetTemperature(-50)
  If test = DRV_SUCCESS Then
    buffer = "Set temperature to -50"
  Else
    vbSetCooling = LOCALERROR
    ...
    Select Case (test)
      ...
      Case DRV_ERROR_ACK
        buffer = buffer & "Unable to communicate with card."
      Case Else
        buffer = buffer & "Unknown temperature error"
    End Select
  End If
  test = CoolerON()
  PrintStatusMsg (buffer)
...
'**************************************************************
  ...
  Dim first As Long
  Dim last As Long
  Dim validfirst As Long
  Dim validlast As Long
  
  test = GetNumberNewImages(first, last)
  If (test <> DRV_SUCCESS) Then
    buffer = "No new data: "
  Else
    test = GetImages(first, first, aData(1), nXPixels, validfirst, validlast)
