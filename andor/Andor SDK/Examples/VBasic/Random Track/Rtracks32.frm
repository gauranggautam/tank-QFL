...
Begin VB.Form Form1 
   BackColor       =   &H00808000&
   Caption         =   "Instaspec Visual Basic Example Drivers:- RandomTracks"
   ClientHeight    =   7245
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8295
   ...
   Begin VB.CommandButton ResetTracks 
      Caption         =   "Reset"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
      ...
      Width           =   615
   End
   Begin VB.CommandButton AddNewTrack 
      Caption         =   "Add"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
   ...
   Begin VB.TextBox EndTrackText 
      ...
      Width           =   972
   End
   Begin VB.TextBox StartTrackText 
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.TextBox ebShutterTime 
      ...
      Width           =   972
   End
   Begin VB.CommandButton pbDisplayTrack 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.TextBox ebCCDTrack 
         ...
         Width           =   732
      End
      Begin VB.Label lblDisplayTrack 
         BackColor       =   &H00808000&
         Caption         =   "Track of the scan   to display on the screen"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Label lblShutterTime 
      BackColor       =   &H00808000&
      Caption         =   "Ti&me to open/close shutter (ms)"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
'acquire a Single Scan, RandomTracks image.

Private Sub Command1_Click()

End Sub

Private Sub AddNewTrack_Click()
  Dim startT As Integer
  Dim endT As Integer
  Dim mybuffers As String
  Dim mybuffere As String
  Dim bInvalid As Boolean
  
  bInvalid = False
  
  
  mybuffers = StartTrackText.text
  startT = Val(mybuffers)
  mybuffere = EndTrackText.text
  endT = Val(mybuffere)
  
  If (startT < 1) Or (endT < 1) Then
    bInvalid = True
  End If

  If (startT >= nYPixels) Or (endT >= nXPixels) Then
    bInvalid = True
  End If
  ...
  If (itrackCounter > 0) Then
    If (endT <= itrackArray(itrackCounter - 1)) Then
      bInvalid = True
    End If
  End If
  
  If (itrackCounter > 0) Then
    If (startT <= itrackArray(itrackCounter - 1)) Then
      bInvalid = True
    End If
  End If

  If (bInvalid = True) Then
    PrintStatusMsg ("* RANDOMTRACKS Invalid Track")
    Exit Sub
  End If

  szTrackData = szTrackData + "("
  szTrackData = szTrackData + mybuffers + "," + mybuffere + ") "
  PrintStatusMsg (szTrackData)
  itrackArray(itrackCounter) = startT
  itrackCounter = itrackCounter + 1
  itrackArray(itrackCounter) = endT
  itrackCounter = itrackCounter + 1
End Sub
...
'the sensor.
'
'The user may specify the exposure time, the number of tracks to
'acquire, track height and offset.  And also the shutter opening
'time and type of TTL pulse to open.
'
...
'**************************************************************
  ...
  pbDisplayTrack.Enabled = False
  newLine = Chr(13) & Chr(10)
  PrintStatusMsg ("* RANDOMTRACKS * SINGLE SCAN * INTERNAL TRIGGER *")
  InitializeInstaSpec         'Initialize InstaSpec system
  itrackCounter = 0
  ReDim itrackArray(0 To 50)
  szTrackData = ""
End Sub
...
' 4) Set binning pattern to RandomTracks.
'**************************************************************
...
Sub ebCCDTrack_Change()
'***********************************************************
'   Prevents user from accidentally enetering an erroneous
'   value.
'***********************************************************
  index = Val(ebCCDTrack.text)
  numTracks = itrackCounter / 2
  If (index < 1 Or index > numTracks) Then
    ebCCDTrack.text = Str(1)
  End If
End Sub
...
' The current working directory IS the directory which contains
' VB.EXE and isdrv.dll
'
...
Function vbSetRandomTracks()
'**************************************************************
' The binning pattern of the InstaSpec system is configured to be
' a set of tracks (ie RandomTrack mode).
'**************************************************************
    
  Dim ino As Integer
  ino = itrackCounter / 2
  vbSetRandomTracks = Not LOCALERROR
  test = SetRandomTracks(ino, itrackArray(0))
  If (test <> DRV_SUCCESS) Then
    vbSetRandomTracks = LOCALERROR
    buffer = "RandomTrack ERROR: "
    Select Case (test)
      Case DRV_P1INVALID
        buffer = buffer & "Number of tracks invalid."
      Case DRV_P2INVALID
        buffer = buffer & "Track positions invalid."
      Case Else
...
'**************************************************************
' Configure InstaSpec binning pattern to RandomTracks.
'**************************************************************
  vbSetReadout = Not LOCALERROR
  test = SetReadMode(2)              'RandomTracks
  If test = DRV_SUCCESS Then
    buffer = "Set Readout Mode to RandomTracks"
  Else
...
'
'This function assumes the array 2D matches the chip format and retrieves
'a full row of the CCD from the 2D array aData.
'*********************************************************************
...
'**************************************************************
  ...
  nTracks = itrackCounter / 2
  test = GetAcquiredData(aData(1, 1), nXPixels * nTracks)
...
'**************************************************************
' Closes down the RandomTracks application
'**************************************************************
  test = ShutDown()
  If (test <> DRV_SUCCESS) Then
...
Sub pbDisplayTrack_Click()
  PlotData
End Sub

Sub pbStartAcq_Click(index As Integer)
'*************************************************************************************
'This routine is the heart of the RandomTracks application.  It configures the InstaSpec
...
'**************************************************************************************
  ...
  If (StartTimer = XOFF) Then
    ...
    If (vbSetRandomTracks() = LOCALERROR) Then
      Exit Sub
    End If
