...
Begin VB.Form Form1 
   BackColor       =   &H00808000&
   Caption         =   "AndorSDK Visual Basic Example Drivers:- Spooling"
   ClientHeight    =   7815
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8295
   ...
   Begin VB.PictureBox Picture1 
      ...
      Width           =   3975
   End
   Begin VB.TextBox ebKineticsTime 
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
      ...
      Width           =   972
   End
   Begin VB.TextBox ebNumKinetics 
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.TextBox ebNumAccum 
      ...
      Width           =   972
   End
   Begin VB.CommandButton pbDisplayScan 
      Caption         =   "&Display Scan"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Label lblDisplayScan 
         BackColor       =   &H00808000&
         Caption         =   "Scan of the kinetic series to display on the screen"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Label stFilename 
      ...
      Width           =   2535
   End
   Begin VB.Label stSpool 
      BackStyle       =   0  'Transparent
      Caption         =   "Spool to disk"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
      ...
      Width           =   2295
   End
   Begin VB.Label lblKineticsTime 
      BackColor       =   &H00808000&
      Caption         =   "&Kinetic cycle time (secs)"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.Label lblAccumTime 
      BackColor       =   &H00808000&
      Caption         =   "Accumulation cycle &time (secs)"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
      ...
      Width           =   2685
   End
   Begin VB.Label lblNumKinetics 
      BackColor       =   &H00808000&
      Caption         =   "# &of scans in kinetic series"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
      ...
      Width           =   2655
   End
   Begin VB.Label lblNumAccum 
      BackColor       =   &H00808000&
      Caption         =   "&# of accumulations per scan"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
'acquire an (accumulated) Kinetic Series of Full Vertically Binned
'scans. It also demonstrates the use of the Spooling functions which
'reduce memory usage by saving intermediate kinetic scans to disk.

'The user may specify the exposure, accumulated and kinetic
'cycle times, along with the number of accumulations per scan
'AND the number of scans in the series. Spooling can be enabled or
'disabled and the filename by which to save each individual scan
'can be modified.
...
'**************************************************************
  ...
  pbDisplayScan.Enabled = False
  newLine = Chr(13) & Chr(10)
  PrintStatusMsg ("* KINETICS MODE * FVB SCANS * INTERNAL TRIGGER *")
...
Sub ebCCDScan_Change()
'********************************************************************
'   Prevents user from accidentally entering an erroneous value
'********************************************************************
  index = Val(ebCCDScan.text)
  nKineticScans = Val(ebNumKinetics.text)
  If (index < 1 Or index > nKineticScans) Then
...
'****************************************************************
  ...
  Else
    ...
    buffer = buffer & newLine & "**Actual Accumulation Cycle Time = " & Str(AccumTime) & " secs"
    buffer = buffer & newLine & "**Actual Kinetic Cycle Time = " & Str(kineticTime) & " secs"
...
'**************************************************************
' Configure AndorSDK to operate in Kinetics mode.
'**************************************************************
  vbSetAcquisition = Not LOCALERROR
  test = SetAcquisitionMode(3)      'Kinetics
  If test = DRV_SUCCESS Then
    buffer = "Set Acquisition Mode to Kinetics"
  Else
...
'*********************************************************************
  kineticTime = Val(ebKineticsTime.text)
  NumKinetics = Val(ebNumKinetics.text)
...
'**************************************************************
' Configure AndorSDK to Full Vertical Binning.
'**************************************************************
  vbSetReadout = Not LOCALERROR
  test = SetReadMode(0)              'FVB
  If test = DRV_SUCCESS Then
...
Function vbSetSpool()
'**************************************************************
' Set up spooling to disk if requested
'**************************************************************
  Dim aPath As String
  vbSetSpool = Not LOCALERROR
  If ckSpool.Value = 1 Then
    aPath = ebFilename.text
    test = SetSpool(1, 0, aPath, 4) 'Enabled, Mode 0, aPath stem and buffer size of 4
    buffer = "Spooling Enabled"
    SpoolUsed = True
    SpoolStem = aPath
  Else
    aPath = ""
    test = SetSpool(0, 0, aPath, 4)
    buffer = "Spooling Disabled"
    SpoolUsed = False
  End If
  If test <> DRV_SUCCESS Then
    vbSetSpool = LOCALERROR
    buffer = "Set Spooling Error: "
    Select Case (test)
      Case DRV_NOT_INITIALIZED
        buffer = buffer & "AndorSDK system isn't initialized."
      Case DRV_ACQUIRING
...
'Thus for a CCD sensor of pixel dimensions 1024x256 the TOTAL number of
'Full Vertically Binned (FVB) scans in the kinetic series must < available
'data array space, ie:
'
'One FVB scan requires 1024 data elements to store the data and so
'       1024*64 = 65536
'ie TOTAL number of FVB scans in kinetic series <= 64, for 1024x256 sensor.
...
Function LoadData()
'**************************************************************
' After Kinetics_GetStatus() confirms the data acquisition is
' COMPLETE then LoadData() gets the data from the AndorSDK
' drivers and sends it to the graph plotting routine.
'
' See comments at top of GetRowCCDData() for details on aData
'**************************************************************
  LoadData = Not LOCALERROR
  If SpoolUsed Then
    PrintStatusMsg ("Acquired Data to Spool files!")
    PlotData
    Exit Function
  End If
  NumInSeries = Val(ebNumKinetics.text)
  Dim NumElements As Long
  NumElements = nXPixels * NumInSeries
  ReDim aData(1 To NumElements)
  test = GetAcquiredData(aData(1), NumElements)
...
Sub GetDataFromFile(index As Long)
'***************************************************************
' Gets a specific kinetic scan from disk if spooling is enabled
'***************************************************************
  Dim filename As String
    
  ReDim aRowData(1 To nXPixels)
  filename = SpoolStem & LTrim$(Str$(index)) & ".dat"
...
Sub pbDisplayScan_Click()
  PlotData
End Sub

Sub pbStartAcq_Click(index As Integer)
'*************************************************************************************
'This routine is the heart of the Kinetics application.  It configures the AndorSDK
