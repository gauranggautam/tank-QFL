...
def start_kdc(SN="27257399", kdc=None):
    """
    Initializes and connects to a Thorlabs Kinesis Motor (e.g., rotation mount).
    ...
    if kdc is not None:
        ...
        try:
            ...
            kdc.open()
            kdc.home(force=True)
            print(f"Connected and homed KDC motor SN: {SN}")
            return kdc
...
def kdc_move_deg(deg, kdc=None):
    ...
    # Conversion factor based on your calibration: 1 degree = 1919.6418... steps
    step = deg * 1919.6418578623391
    kdc.move_to(step)
    
    if kdc_local:
        # Note: Depending on your workflow, you may want to keep the connection open.
        # Remove or manage kdc.close() if you prefer keeping it active.
        pass
...
def run_pl_polarization(start_deg=0, end_deg=360, step_deg=10, 
                        ...
                        show_plot=True, kdc=None, amc=None, sn=None, d1=None, d2=None):
    """
    ...
    try:
        ...
        # === Plot Results ===
        fig, ax = plt.subplots(subplot_kw={'projection': 'polar'} if True else None, figsize=(8, 6))
        
        # Plotting in Cartesian or Polar based on preference (using Cartesian here for clarity, can adapt)
